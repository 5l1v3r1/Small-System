...
SD_Error SD_Init(void)
{
	...
	GPIOD->CRL|=0X00000B00;	//PD2复用输出,PD7上拉输入
	...
	SDIO->MASK=0x00000000;	  
 	NVIC_Init_(0,0,SDIO_IRQChannel,5);//SDIO中断配置
	...
	{  		    
		...
		errorstatus=SD_SetDeviceMode(SD_POLLING_MODE);			//设置为DMA模式
 	}
	return errorstatus;		 
}
/**
  * @brief: SDIO时钟初始化设置
  * @param: clkdiv:时钟分频系数
  * @param: CK时钟=SDIOCLK/[clkdiv+2];(SDIOCLK时钟直接就是AHB时钟,一般为72Mhz)
  */
...
/**
  ...
  * @param: waitrsp:期待的相应.00/10,无响应;01,短相应;11,长响应
  * @param: arg:参数
  * @retval:
  * @note:
  */
...
/**
  * @brief 卡上电，查询所有SDIO接口上的卡设备,并查询其电压和配置时钟 
  * @param none
	* @note
  * @retval 0 - OK，!0 - 错误码
  */
...
/**
  * @brief 卡选择，发送CMD7,选择相对地址(rca)为addr的卡,取消其他卡.如果为0,则都不选择
  * @param addr - 卡的RCA地址
	* @note
  * @retval 0 - OK，!0 - 错误码
  */
...
/**
  ...
  * @param: mbuf:存储器地址
  * @param: bufsize:传输数据量
  * @param: dir:方向;1,存储器-->SDIO(写数据);0,SDIO-->存储器(读数据);
  * @retval:
  * @note:
  */
